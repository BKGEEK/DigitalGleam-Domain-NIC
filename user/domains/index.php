...
$manageDomainId = isset($_GET['manage_ns']) ? (int) $_GET['manage_ns'] : 0;
$manageDomain = null;
$nsRecords = [];
if ($manageDomainId > 0) {
    foreach ($rows as $row) {
...
$title = $manageDomain ? 'NS 记录管理 - ' . htmlspecialchars(dns_domain_display_name($manageDomain)) : '我的域名';
$activeKey = 'domains';

user_render($title, $activeKey, function () use ($rows, $manageDomain, $manageDomainId, $nsRecords, $message, $messageType, $pdo, $userId): void {
    ?>
    <section class="panel">
        <?php if ($manageDomain): ?>
