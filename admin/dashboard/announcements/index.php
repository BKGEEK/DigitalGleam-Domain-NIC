...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            ...
            if ($title === '' || $content === '') {
                throw new RuntimeException('请填写标题和内容。');
            }
...
admin_dashboard_render('公告管理', 'announcements', function () use ($rows, $error, $message): void {
    ?>
    <div class="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <section class="panel">
            <h1 class="text-2xl font-semibold text-slate-900">公告管理</h1>
            <p class="mt-3 text-sm text-slate-600">这里管理公告发布、显示和删除。</p>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <thead class="bg-slate-50 text-left text-slate-500">
                        <tr>
                            ...
                            <th class="px-4 py-3 font-medium">发布人</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                ...
                                <td class="px-4 py-3 text-slate-600"><?= (int) $row['status'] === 1 ? '显示' : '隐藏' ?></td>
                                ...
                                <td class="px-4 py-3 space-x-2">
                                    <form method="post" class="inline">
                                        ...
                                        <button class="text-brand-700 hover:text-brand-800" type="submit"><?= (int) $row['status'] === 1 ? '隐藏' : '显示' ?></button>
                                    </form>
                                    <form method="post" class="inline">
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="id" value="<?= (int) $row['id'] ?>">
                                        <button class="text-red-600 hover:text-red-700" type="submit">删除</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
        ...
        <section class="panel">
            <h2 class="text-xl font-semibold text-slate-900">发布公告</h2>
            <form method="post" class="mt-6 space-y-4">
                ...
                <div>
                    <label class="mb-2 block text-sm font-medium text-slate-700">内容</label>
                ...
                <button type="submit" class="btn-primary w-full justify-center">发布公告</button>
            </form>
        </section>
    </div>
    <?php
